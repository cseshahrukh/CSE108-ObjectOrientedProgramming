...
public:
    ...
    ContainerArray(int p)
    {
        allocatedSize=p;
        arrayOfContainers=new Container[p];
    }
    //my code done
    ...
    void setItemAt(Container a, int index)
    {
        ...
        {
            if (a.getStoredType()==1)
            {
                int *v=(int*) a.getItem();
                arrayOfContainers[index].setItem( *v);
            }
            else if (a.getStoredType()==2)
            {
                int *v=(int*) a.getItem();
                arrayOfContainers[index].setItem(v, a.getFirstDim());
            }
            else if (a.getStoredType()==3)
            {
                int **v=(int**)a.getItem();
                arrayOfContainers[index].setItem(v, a.getFirstDim(), a.getSecondDim());
            }
            else if (a.getStoredType()==-1)
            {
                 arrayOfContainers[index].setItem();
            }
        }
    }
    //my code ends
